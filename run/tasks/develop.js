...
function run()
{
  ...
  return [
    ...
    `concurrently -rs first '${buildRunnables.map(script => script.replace(/'/g, '\"')).join(`' '`)}'`,
    ...
    `concurrently -c blue,green,magenta,cyan,yellow -p time -t 'HH:mm:ss' '${watchRunnables.map(script => script.replace(/'/g, '\"')).join(`' '`)}'`
  ]
}
